...
export default {
    ...
    setController_table: function (core) {
        ...
        tableResize.innerHTML = '' +
            ...
            '</button>' +
            // '<button type="button" data-command="settings" class="se-btn se-tooltip">' +
            // icons.settings +
            // '<span class="se-tooltip-inner"><span class="se-tooltip-text">' + 'Table settings' + '</span></span>' +
            // '</button>' +
            '</div>' +
            '</div>';
    ...
    appendTable: function () {
        ...
        oTable.style.tableLayout = 'auto !important';
        oTable.style.border = '1px solid rgb(204, 204, 204)';
        oTable.style.width = '100%';
        oTable.style.maxWidth = '100%';
        oTable.style.margin = '0 0 10px';
        ...
        let rowStyle = 'style="border: 1px solid #efefef;"';
        let cellStyle = 'style="border: 1px solid #e1e1e1;padding:0.4em;background-clip: padding-box;"';
        ...
        let tableHTML = '<tbody>';
        while (y > 0) {
            //tableHTML += '<tr >' + createCells.call(this, 'td', x, cellStyle) + '</tr>';
            tableHTML += `<tr ${rowStyle}>${createCells.call(this, 'td', x, false, cellStyle)}</tr>`;
            --y;
        }
        tableHTML += '</tbody>';
    ...
    createCells: function (nodeName, cnt, returnElement, style = '') {
        ...
        if (!returnElement) {
            ...
            while (cnt > 0) {
                ...
                cellsHTML += `<${nodeName} ${style}><div><br></div></${nodeName}>`;
                cnt--;
            }
            return cellsHTML;
        } else {
    ...
    init: function () {
        ...
        if (tablePlugin._selectedTable) {
            ...
            for (let i = 0, len = selectedCells.length; i < len; i++) {
                ...
                console.log('se-table-selected-cell');
            }
        }

        if (tablePlugin._columnResizeHandler) {
            tablePlugin._columnResizeHandler.dispose();
            tablePlugin._columnResizeHandler = null;
        }
    ...
    toggleHeader: function () {
        ...
        let rowStyle = 'style="border: 1px solid #efefef;"';
        let cellStyle = 'style="border: 1px solid #e1e1e1;padding:0.4em;background-clip: padding-box;background-color: #f3f3f3;"';
        ...
        if (!active) {
            ...
            header.innerHTML = `<tr rowStyle>${this.plugins.table.createCells.call(this, 'th', this.context.table._logical_cellCnt, false, cellStyle)}</tr>`;
    ...
    _removeEvents: function (contextTable, e) {
        const tablePlugin = this.plugins.table;
    ...
    onTableCellMultiSelect: function (tdElement, shift) {
        ...
        tablePlugin._columnResizeHandler = tablePlugin._onColumnResizeHandler(this, tablePlugin._selectedTable);
        tablePlugin._columnResizeHandler.init();

        // tablePlugin._bindOnColumnResize = tablePlugin._onColumnResize.bind(this);
        // tablePlugin._bindOnColumnResize.call(this, tablePlugin._selectedTable);


        // tablePlugin._bindOnResize = tablePlugin._onColumnResize.bind(this);

        // debugger;

        // const allRows = tablePlugin._selectedTable.querySelectorAll('tr');
        // for (let i = 0; i < allRows.length; i++) {
        //     let allCells = tablePlugin._selectedTable.querySelectorAll('th, td');
        //     for (let j = 0; j < allRows.length; j++) {
        //         let span = document.createElement("span");
        //         span.setAttribute("data-resizecol", j);
        //         span.addEventListener('mousedown', tablePlugin._bindOnResize, false);
        //         allCells[j].style.position = 'relative';
        //         allCells[j].appendChild(span);
        //     }
        // }
    ...
    onClick_tableController: function (e) {
        ...
        switch (command) {
            ...
            case 'settings':
                tablePlugin.tableSettings.call(this);
                break;
        }
    ...
    _bindOnColumnResize: null,
    _onColumnResize: function (table) {
        const me = this;
        ...
        function setListeners(div) {
            ...
            const mouseUpHandler = function (e) {
                ...
                me._wd.removeEventListener('mousemove', mouseMoveHandler);
                me._wd.removeEventListener('mouseup', mouseUpHandler);

                me.history.push(false);
            };
            ...
            const mouseDownHandler = function (e) {
                ...
                me._wd.addEventListener('mousemove', mouseMoveHandler);
                me._wd.addEventListener('mouseup', mouseUpHandler);
            };
        ...
        function createDiv(height, index) {
            ...
            div.classList.add("se-data-resizecol");
            // div.style.top = 0;
            // div.style.right = 0;
            // div.style.width = '5px';
    ...
    _onColumnResizeHandler: function (core, table) {
        ...
        function setListeners(div) {
            ...
            const mouseUpHandler = function (e) {
                ...
                core._wd.removeEventListener('mousemove', mouseMoveHandler);
                core._wd.removeEventListener('mouseup', mouseUpHandler);
            ...
            mouseDownHandler = function (e) {
                ...
                core._wd.addEventListener('mousemove', mouseMoveHandler);
                core._wd.addEventListener('mouseup', mouseUpHandler);
            };

            div.addEventListener('mousedown', mouseDownHandler);
        }
        function clean() {
            var cols = table.querySelectorAll('.se-data-resizecol');
            if (!cols) return;
            for (var i = 0; i < cols.length; i++) {
                const col = cols[i];
                col.removeEventListener('mousedown', mouseDownHandler);
                col.remove();
            }
        }
