...
export default class SMST032VM extends cntrl.VMBase {
    constructor(props) {
        super(props);
    ...
    init() {
        ...
        model.GridInfo.Columns = [
            ...
            { text: 'Type Available', field: 'dlvry_typ_aval', width: '30%' },
    ...
    loadSelectedData(dlvry_typ_id) {
        ...
        cntrl.Utils.ajax({ url: `${this._WebApi}/SMST032LoadSelectedDataAsync`, data: { dlvry_typ_id } }, (r) => {
            ...
            dataModel.WKG_mrkup_avail_F = r.dlvry_wkg_markup || "";
            dataModel.Del_avail_F = r.dlvry_typ_aval ? "Yes" : "No";
    ...
    loadPage(pageIndex, columnOptions = null) {
        ...
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SMST032SearchAsync`, data: dataInfo, files: [] }, (r) => {
            try {
                model.Loading = false;
    ...
    handleSave() {
        ...
        if (this.isValueChanged()) {
            const dataInfo = {
                ...
                dlvry_typ_nam: model.Delivery_name_F,
                dlvry_shrt_nam: model.Short_name_F,
                act_inact_ind: model.IsActiveF,
            }
            cntrl.Utils.ajax({ url: `${this._WebApi}/SMST032SaveDataAsync`, data: dataInfo }, (r) => {
                if (r.IsSuccess) {
                    const opts = {
    ...
    setTitle() {
        const model = this.Data;
        model.Title = `${this.props.data.Title} / Edit ${model.Input.Delivery_name_F === "" ? "" : `/ ${model.Input.Delivery_name_F}`}`;
    }
    getData() {
        const model = this.Data.Input;
        const dataInfo = {};
