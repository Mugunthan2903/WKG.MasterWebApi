...
export default class SSM150VM extends VMBase {
    constructor(props) {
        super(props);
        this._WebApi = 'SSM150';
        this.init();
    ...
    init() {
        ...
        model.GridInfo = {
            ...
            Columns: [
                ...
                { text: 'Differential Name', field: 'diff_nam', width: '45%', sort: { enabled: true } },
                { text: 'Location Types', field: 'loc_typ_nam', width: '30%' },
    ...
    IniloadPage(pageIndex, columnOptions = null, loading = true) {
        ...
        this.updateUI();
        Utils.ajax({ url: `${this._WebApi}/SSM150OnloadSearch`, data: dataInfo }, (r) => {
            try {
                model.Loading = false;
                if (r) {
                    if (r.Items)
                ...
                console.error("Error in SSM150 onload : ", ex);
            } finally {
                me.updateUI();
            }
        });
    ...
    doSearchClear(clearAll = false) {
        ...
        const gridInfo = model.GridInfo;
        if (clearAll === true) {
            model.SearchInput.DifferentialName = "";
            model.SearchInput.Status_Srch = true;
            this.setFocus("DifferentialName");
        }
        gridInfo.Items = [];
        this.Data.DataCopyGrid = JSON.stringify(gridInfo.Items.map(e => ({ IsSelected: e.IsSelected })));
    ...
    doSave(e) {
        ...
        Utils.ajax({ url: `${this._WebApi}/SSM150DifferentialSaveGrid`, data: dataInfo }, r => {
            try {
                model.Loading = false;
                r = r || {};
                if (r.IsSuccess === true) {
                ...
                console.error("Error in SSM150 Save : ", ex);
            }
            finally {
                me.updateUI();
            }
    ...
    openNewWindow(Sload, pid, pname) {
        ...
        if (Sload === "Fare") {
            this.showWindow({
                url: 'SSM/SSM151', data: { Title: model.Title, Crrg_ID: pid, Crrg_Name: pname, SuppMapID: model.DBSuppID, ImageDirectory: model.DBImgdir }, windowStyle: WKLWindowStyles.slideLeft, onClose: (e) => {
