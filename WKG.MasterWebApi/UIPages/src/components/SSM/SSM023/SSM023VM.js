...
export default class SSM023VM extends cntrl.VMBase {
    constructor(props) {
        super(props);
    ...
    onLoad() {
        ...
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM023OnLoadDataAsync`, data: dataInfo, files: [] }, (r) => {
            model.Loading = false;
            if (r) {
                try {
    ...
    loadPage(pageIndex, columnOptions = null) {
        ...
        model.AllSelected = false;
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM023SearchDataAsync`, data: dataInfo }, (r) => {
            try {
                model.Loading = false;
    ...
    onBlurSearch() {
        ...
        dataInfo.supp_map_id = model.SuppMapID;
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM023BlurSrchAsync`, data: dataInfo, files: [] }, (r) => {
            try {
    ...
    loadSelectedData(excptn_srl) {
        ...
        model.Loading = true;
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM023LoadSelectedData`, data: { excptn_srl } }, (r) => {
            model.Loading = false;
            if (r) {
    ...
    doSave(e) {
        if (this.isvalidSave(e)) {
            ...
            model.Loading = true;
            me.updateUI();
            cntrl.Utils.ajax({ url: `${this._WebApi}/SSM023SaveDataAsync`, data: dataInfo }, r => {
                try {
                    model.Loading = false;
                    r = r || {};
                    if (r.IsSuccess === true) {
    ...
    setTitle() {
        const model = this.Data;
        if (model.Input.IsEdit) {
            model.Title = `Tui Product / Exception / ${model.prod_name} / Edit ${cntrl.Utils.isNullOrEmpty(model.Input.group_name) ? "" : "/ " + model.Input.group_name.Text}`;
        } else {
            model.Title = `Tui Product / Exception / ${model.prod_name} / New`;
        }
    };
