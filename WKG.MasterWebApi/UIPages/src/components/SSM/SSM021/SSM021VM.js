...
export default class SSM021VM extends VMBase {
    constructor(props) {
        super(props);
    ...
    init() {
        ...
        model.GridInfo = {
            ...
            Columns: [
                { text: 'Name', field: 'tui_city_nam', width: '30%', sort: { enabled: true } },
                ...
                { text: 'Delisted ', field: 'tui_city_aval', width: '20%' },
            ]
        };
        this.newMode();
    }
    ...
    getData() {
        ...
        const dataInfo = {};
        dataInfo.tui_city_cd = model.City_ID;
        dataInfo.tui_city_nam = model.City_Name;
        dataInfo.supp_map_id = dataModel.SupplierMapID;
        if (!Utils.isNullOrEmpty(model.WKG_City)) {
            dataInfo.wkg_city_cd = model.WKG_City?.ID;
        }
        dataInfo.act_inact_ind = model.Status === true ? 'True' : "False";
    ...
    loadInitData() {
        ...
        dataInfo.tui_city_nam = model.SearchInput.ScrhcityName;
        dataInfo.act_inact_ind = model.SearchInput.Status_Srch;
        dataInfo.tui_city_aval = !model.SearchInput.city_avail_Srch;
        model.Loading = true;
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM020GetTuiProductsAsync`, data: dataInfo, files: [] }, (r) => {
            try {
                model.Loading = false;
                if (r) {
    ...
    fillSearchResult(r, selectedItem = null) {
        ...
        gridInfo.Items = r.Items.map(e => ({
            ...
            tui_city_aval: e.tui_city_aval === "True" ? "No" : "Yes",
    ...
    loadPage(pageIndex, columnOptions = null, onload = false) {
        ...
        dataInfo.supp_map_id = model.SupplierMapID;
        dataInfo.tui_city_nam = model.SearchInput.ScrhcityName;
        dataInfo.tui_city_aval = model.SearchInput.city_avail_Srch === true ? 0 : 1;
        dataInfo.act_inact_ind = model.SearchInput.Status_Srch === true ? 1 : 0;
        dataInfo.SortTyp = true;
        columnOptions = columnOptions || [];
        if (columnOptions.length > 0) {
            for (const itm of columnOptions) {
                if (itm.field === "tui_city_nam" && !cntrl.Utils.isNullOrEmpty(itm.sort)) {
    ...
    setSelectedItem(selectedItem, loadData = false) {
        ...
        if (loadData === true)
            this.loadSelectedData(selectedItem.tui_city_cd);
    }
    loadSelectedData(id) {
        const me = this;
        const model = this.Data;
        const dataInfo = {};
        dataInfo.tui_city_cd = id;
        dataInfo.supp_map_id = model.SupplierMapID;
        model.Loading = true;
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM021GetEditTUICity`, data: dataInfo }, (r) => {
            try {
                model.Loading = false;
                if (r) {
                    model.Input.City_ID = r.tui_city_cd;
                    model.Input.City_Name = r.tui_city_nam;
                    if (r.wkg_city_cd) {
                    ...
                    model.Input.Status = r.act_inact_ind.toUpperCase() == "TRUE" ? true : false;;
    ...
    handleDataChange(selectedItem) {
        ...
        const model = this.Data.Input;
        if (selectedItem) {
            if (selectedItem.tui_city_cd === model.City_ID) {
