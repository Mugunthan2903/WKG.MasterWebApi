...
export default class SSM132VM extends cntrl.VMBase {
    constructor(props) {
        super(props);
    ...
    onLoad() {
        ...
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM132OnLoadExcep`, data: dataInfo, files: [] }, (r) => {
            model.Loading = false;
            if (r) {
                try {
                    ...
                    console.error("Error in SSM132 onload exception: ", ex);
                }
                finally {
                    me.newMode(false);
                    me.updateUI();
    ...
    loadPage(pageIndex, columnOptions = null) {
        ...
        model.AllSelected = false;
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM132SearchExcep`, data: dataInfo }, (r) => {
            try {
                model.Loading = false;
                me.fillSearchResult(r || {}, selectedItem);
            }
            catch (ex) {
                console.error("Error in SSM132 search exception: ", ex);
            }
            finally {
                me.updateUI();
            }
    ...
    onBlurSearch() {
        ...
        dataInfo.supp_map_id = model.SuppMapID;
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM132BlurSrchExcep`, data: dataInfo, files: [] }, (r) => {
            try {
                ...
                console.error("Error in SSM132 blur search exception: ", ex);
            }
            finally {
                me.updateUI();
            }
    ...
    loadSelectedData(excptn_srl) {
        ...
        model.Loading = true;
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM132LoadSelectExcep`, data: { excptn_srl } }, (r) => {
            model.Loading = false;
            try {
                if (r) {
                ...
                console.error("Error in SSM132 load select exception: ", ex);
            }
            finally {
                me.updateUI();
            }
    ...
    doSave(e) {
        if (this.isvalidSave(e)) {
            ...
            dataInfo.supp_map_id = model.SuppMapID;
            dataInfo.EXSavedata = model.Input.IsEdit ? "UPDATE" : "INSERT";
            model.Loading = true;
            me.updateUI();
            cntrl.Utils.ajax({ url: `${this._WebApi}/SSM132SaveExcepData`, data: dataInfo }, r => {
                try {
                    model.Loading = false;
                    r = r || {};
                    if (r.IsSuccess === true) {
                    ...
                    console.error("Error in SSM132 save exception: ", ex);
                }
                finally {
                    me.updateUI();
                }
    ...
    setTitle() {
        const model = this.Data;
        if (model.Input.IsEdit) {
            model.Title = `${this.props.data.Title} / Exception / ${model.prod_name} / Edit ${cntrl.Utils.isNullOrEmpty(model.Input.group_name) ? "" : "/ " + model.Input.group_name.Text}`;
        } else {
            model.Title = `${this.props.data.Title} / Exception / ${model.prod_name} / New`;
        }
    };
