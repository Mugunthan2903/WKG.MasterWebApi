...
export default class SSM051VM extends cntrl.VMBase {
    constructor(props) {
        super(props);
    ...
    init() {
        ...
        model.GridInfo.Columns = [
            ...
            { text: 'Name', field: 'bg_prod_nam', width: '70%' },
            { text: '', field: '', width: '10%' }
    ...
    loadPage(pageIndex, loader = true, loadData = true) {
        ...
        const dataInfo = {};
        dataInfo.bg_prod_id = model.Input.Prod_ID;
        ...
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM051_Gridbinding`, data: dataInfo, files: [] }, (r) => {
            try {
                model.Loading = false;
                if (r) {
                    if(loadData){
                    ...
                    if (r.GetOvrride.length !== 0 && loadData) {
                        ...
                        //model.Input.Prod_Name = r.GetOvrride[0].lp_prod_nam;
    ...
    doSave(e) {
        ...
        model.Loading = true;
        me.updateUI();
        Utils.ajax({ url: `${this._WebApi}/SSM051SaveDataImg`, data: dataInfo, files: model.Image_Array || [] }, r => {
            try {
                model.Loading = false;
                r = r || {};
                if (r.IsSuccess === true) {
    ...
    //     model.Loading = true;
    //     me.updateUI();
    //     Utils.ajax({ url: `${this._WebApi}/SSM051RemoveImage`, data: dataInfo, files: '' || [] }, r => {
    //         try {
    //             model.Loading = false;
    //             r = r || {};
    //             if (r.IsSuccess === true) {
    ...
    setTitle() {
        const model = this.Data;
        const dataModel = this.Data.Input;
        model.Title = `Big Bus Product / Edit / ${dataModel.Prod_Name} `;
