...
export default class SSM085VM extends VMBase {
    constructor(props) {
        super(props);
    ...
    loadInitData(load = true) {
        ...
        model.Loading = true;
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM085OnLoadData`, data: dataInfo }, (r) => {
            try {
                model.Loading = false;
    ...
    onDeleteclick = (note_srl) => {
        ...
        this.Data.DataCopyNotes = JSON.stringify(dataCopyNotes);
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM085DeleteData`, data: dataInfo }, (r) => {
            try {
                model.Loading = false;
                if (r.IsSuccess) {
    ...
    handleDeleteFollowUp(note_srl) {
        ...
        this.showAlert("Note deleted successfully" ,() => this.updateUI());
        const dataCopyNotes = this.getDataNotes();
        this.Data.DataCopyNotes = JSON.stringify(dataCopyNotes);
    }
    renderNotes(info) {
    ...
    onBlurSrch() {
        ...
        this.updateUI();
        Utils.ajax({ url: `${this._WebApi}/SSM085OnBlurSearch`, data: dataInfo, files: [] }, (r) => {
            try {
                r = r || {};
    ...
    isvalidSave(e) {
        ...
        let noteFlag = true;
        if (Utils.isNullOrEmpty(model.Language)) {
            this.showAlert('Please Select Language', () => this.setFocus('Language'));
            noteFlag = false;
            return noteFlag;
        }
        temp.forEach(e => {
    ...
    handleSave(e) {
        ...
        if (this.isValueChanged() || this.isNotesChanged()) {
            ...
            if (model.Input.IsEdit) {
                me.showAlert("No changes has been made.", () => this.setFocus('Prod_Name'));
            }
            else {
                me.showAlert("Please Enter required fields.", () => this.setFocus('Language'));
            }
    ...
    doSave(e) {
        if (this.isvalidSave(e)) {
            ...
            model.Loading = true;
            me.updateUI();
            Utils.ajax({ url: `${this._WebApi}/SSM085SaveDataAsync`, data: dataInfo }, r => {
                try {
                    model.Loading = false;
                    r = r || {};
                    if (r.IsSuccess === true) {
