...
export default class SSM061VM extends cntrl.VMBase {
    constructor(props) {
        super(props);
    ...
    loadPage(pageIndex, loader = true) {
        ...
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM061OnLoadDataAsync`, data: dataInfo, files: [] }, (r) => {
            try {
                model.Loading = false;
                if (r && loader) {
                    ...
                    model.Input.bkfee_type = r.PrdOvrdDtl.bkng_fee_typ === null ? r.radios.Percentage : (r.PrdOvrdDtl.bkng_fee_typ);
    ...
    doSave() {
        ...
        model.Loading = true;
        me.updateUI();
        Utils.ajax({ url: `${this._WebApi}/SSM061SaveDataAsync`, data: dataInfo }, r => {
            try {
                model.Loading = false;
                r = r || {};
                if (r.IsSuccess === true) {
    ...
    setTitle() {
        const model = this.Data;
        const dataModel = this.Data.Input;
        model.Title = `London Pass Product / Edit / ${dataModel.Prod_Name} `;
