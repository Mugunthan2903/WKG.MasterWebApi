...
export default class SSM080VM extends VMBase {
    constructor(props) {
        super(props);
        this._WebApi = 'SSM080';
        this.init();
    ...
    loadPage(pageIndex, columnOptions = null, loading = true) {
        ...
        this.updateUI();
        Utils.ajax({ url: `${this._WebApi}/SSM080GetOnloadScrh`, data: dataInfo }, (r) => {
            try {
                if(r){
                    console.log(r);
    ...
    doSave(e) {
        ...
        model.Loading = true;
        me.updateUI();
        Utils.ajax({ url: `${this._WebApi}/SSM080SaveExcDataAsync`, data: dataInfo }, r => {
            try {
                model.Loading = false;
                r = r || {};
                if (r.IsSuccess === true) {
    ...
    setTitle() {
        const model = this.Data;
        model.Title = model.Input.IsEdit ? `Explorer Pass Product / Edit / ${model.Input.SSM080_Mpid.Text}` : `Explorer Pass Product`;
    }
    ...
    openShowWindow(Type, pid, pname) {
        ...
        if (Type === "Exception") {
            this.showWindow({
                url: 'SSM/SSM082', data: { InputData: { pid, pname, SuppMapID:model.SupplierMapID } }, windowStyle: WKLWindowStyles.slideLeft, onClose: (e) => {
                }
            });
        } else if (Type === "Edit_grid") {
            this.showWindow({
                url: 'SSM/SSM081', data: { Prod_ID: pid, Prod_Name: pname, SuppMapID:model.SupplierMapID }, windowStyle: WKLWindowStyles.slideLeft, onClose: (e) => {
            ...
            this.showWindow({
                url: 'SSM/SSM084', data: { Prod_ID: pid, Prod_Name: pname, SuppMapID:model.SupplierMapID, ImageDirectory:model.ImageDirectory }, windowStyle: WKLWindowStyles.slideLeft, onClose: (e) => {
                }
            });
        }
