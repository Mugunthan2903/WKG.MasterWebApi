...
export default class SSM084VM extends cntrl.VMBase {
    constructor(props) {
        super(props);
    ...
    loadPage(pageIndex, loader = true, loadData = true) {
        ...
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM084OnLoadData`, data: dataInfo, files: [] }, (r) => {
            try {
                model.Loading = false;
                if (r) {
                    if(loadData){
    ...
    doSave(e) {
        ...
        model.Loading = true;
        me.updateUI();
        Utils.ajax({ url: `${this._WebApi}/SSM084SaveImageData`, data: dataInfo, files: model.Image_Array || [] }, r => {
            try {
                model.Loading = false;
                r = r || {};
                if (r.IsSuccess === true) {
    ...
    //     model.Loading = true;
    //     me.updateUI();
    //     Utils.ajax({ url: `${this._WebApi}/SSM084RemoveImage`, data: dataInfo, files: '' || [] }, r => {
    //         try {
    //             model.Loading = false;
    //             r = r || {};
    //             if (r.IsSuccess === true) {
    ...
    setTitle() {
        const model = this.Data;
        model.Title = `Explorer Pass Info`;
    ...
    openWindow(type, InputObj) {
        ...
        if (type == "sec2_New") {
            ...
            Url = 'SSM/SSM009';
            IsEdit = true;
            let lp_prod_typ = model.LP_Prod_Type.ExplorerPass;
            this.showWindow({
