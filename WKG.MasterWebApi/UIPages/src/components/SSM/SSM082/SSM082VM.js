...
export default class SSM082VM extends cntrl.VMBase {
    constructor(props) {
        super(props);
    ...
    onLoad() {
        ...
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM082LoadInitDataAsync`, data: dataInfo, files: [] }, (r) => {
            model.Loading = false;
            if (r) {
                try {
                    model.DefaultMarkupType = r.radios;
    ...
    loadPage(pageIndex, columnOptions = null) {
        ...
        model.AllSelected = false;
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM082SearchDataAsync`, data: dataInfo }, (r) => {
            try {
                model.Loading = false;
    ...
    loadSelectedData(excptn_srl) {
        ...
        model.Loading = true;
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM082LoadSelectedDataAsync`, data: { excptn_srl } }, (r) => {
            model.Loading = false;
            if (r) {
    ...
    doSave(e) {
        if (this.isvalidSave(e)) {
            ...
            model.Loading = true;
            me.updateUI();
            cntrl.Utils.ajax({ url: `${this._WebApi}/SSM080SaveExcDataAsync`, data: dataInfo }, r => {
                try {
                    model.Loading = false;
                    r = r || {};
                    if (r.IsSuccess === true) {
    ...
    onBlurSearch() {
        ...
        dataInfo.supp_map_id = model.SuppMapID;
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM082BlurAsync`, data: dataInfo }, (r) => {
            try {
    ...
    setTitle() {
        const model = this.Data;
        if (model.Input.IsEdit) {
            model.Title = `Explorer Pass Product / Exception / ${model.prod_name} / Edit ${cntrl.Utils.isNullOrEmpty(model.Input.group_name) ? "" : "/ " + model.Input.group_name.Text}`;
        } else {
            model.Title = `Explorer Pass Product / Exception / ${model.prod_name} / New`;
        }
    };
