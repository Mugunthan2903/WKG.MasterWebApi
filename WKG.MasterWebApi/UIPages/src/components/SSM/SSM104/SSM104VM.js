...
export default class SSM104VM extends cntrl.VMBase {
    constructor(props) {
        super(props);
    ...
    onLoad() {
        ...
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM104OnLoadExcep`, data: dataInfo, files: [] }, (r) => {
            model.Loading = false;
            if (r) {
                try {
    ...
    loadPage(pageIndex, columnOptions = null) {
        ...
        model.AllSelected = false;
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM104SearchExcep`, data: dataInfo }, (r) => {
            try {
                model.Loading = false;
    ...
    onBlurSearch() {
        ...
        dataInfo.supp_map_id = model.SuppMapID;
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM104BlurSrchExcep`, data: dataInfo, files: [] }, (r) => {
            try {
    ...
    loadSelectedData(excptn_srl) {
        ...
        model.Loading = true;
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM104LoadSelectExcep`, data: { excptn_srl } }, (r) => {
            model.Loading = false;
            if (r) {
    ...
    doSave(e) {
        if (this.isvalidSave(e)) {
            ...
            model.Loading = true;
            me.updateUI();
            cntrl.Utils.ajax({ url: `${this._WebApi}/SSM104SaveExcepData`, data: dataInfo }, r => {
                try {
                    model.Loading = false;
                    r = r || {};
                    if (r.IsSuccess === true) {
