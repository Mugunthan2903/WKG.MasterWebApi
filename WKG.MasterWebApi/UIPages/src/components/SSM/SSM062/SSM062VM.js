...
export default class SSM062VM extends cntrl.VMBase {
    constructor(props) {
        super(props);
    ...
    onLoad() {
        ...
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM062LoadInitDataAsync`, data: dataInfo, files: [] }, (r) => {
            model.Loading = false;
            if (r) {
                try {
                    model.Group_Name_List = r.comboList.filter(e => e.act_inact_ind === true).map(e => ({ ID: e.pos_grp_id, Text: e.pos_grp_nam }));
                    model.Group_Name_ListAll = r.comboList.map(e => ({ ID: e.pos_grp_id, Text: e.pos_grp_nam }));
    ...
    fillSearchResult(r, selectedItem = null) {
        const model = this.Data;
        const gridInfo = model.GridInfo;
        gridInfo.Items = r.Items.map(e => ({ ...e, act_inact_ind: e.act_inact_ind ? "Active" : "Inactive", wkg_markup_typ: e.wkg_markup_typ === model.radios.percentage ? 'Percentage' : 'Fixed' })) || [];
    ...
    loadPage(pageIndex, columnOptions = null) {
        ...
        model.AllSelected = false;
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM062SearchDataAsync`, data: dataInfo }, (r) => {
            try {
                model.Loading = false;
    ...
    loadSelectedData(excptn_srl) {
        ...
        model.Loading = true;
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM062LoadSelectedDataAsync`, data: { excptn_srl } }, (r) => {
            model.Loading = false;
            if (r) {
                datamodel.IsEdit = true;
                model.excptn_srl = r.excptn_srl;
                datamodel.group_name = model.Group_Name_ListAll.find(e => e.ID === r.pos_grp_id);
                datamodel.Markup_Amount = r.wkg_markup || "";
                datamodel.mrkup_typ = r.wkg_markup_typ;
    ...
    doSave(e) {
        if (this.isvalidSave(e)) {
            ...
            dataInfo.Mode = model.Input.IsEdit ? "EXUPDATE" : "EXINSERT";
            dataInfo.supp_map_id = model.SuppMapID;
            model.Loading = true;
            me.updateUI();
            cntrl.Utils.ajax({ url: `${this._WebApi}/SSM060SaveExcDataAsync`, data: dataInfo }, r => {
                try {
                    model.Loading = false;
                    r = r || {};
                    if (r.IsSuccess === true) {
    ...
    onBlurSearch() {
        ...
        dataInfo.supp_map_id = model.SuppMapID;
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM062BlurAsyncAsync`, data: dataInfo }, (r) => {
            try {
    ...
    setTitle() {
        const model = this.Data;
        if (model.Input.IsEdit) {
            model.Title = `London Pass Product / Exception / ${model.prod_name} / Edit ${cntrl.Utils.isNullOrEmpty(model.Input.group_name) ? "" : "/ " + model.Input.group_name.Text}`;
        } else {
            model.Title = `London Pass Product / Exception / ${model.prod_name} / New`;
        }
    };
    ...
    getData() {
        ...
        const dataInfo = {
            ...
            act_inact_ind: model.Input.IsActive,
