...
import SSM130VM from './SSM130VM';
import * as cntrl from '../../../wkl-components';
import { fontSize } from '../../../wkl-components/WKLEditor/src/plugins';

export default class SSM130 extends cntrl.WKLComponent {
    constructor(props) {
        super(props, new SSM130VM(props));
        this.inputRefs = {};
    }
    onLoad = () => {
    ...
    onPageChange = (e) => {
        if (this.VM)
            this.VM.handleSearch(e.value, e.columnOptions, true);
    };
        ...
        if (this.VM) {
            if (e) {
                ...
                if (action === "btn_search") {
                    ...
                    this.VM.openWindow("Edit", e.pid, e.pname);
                }
                else if (action === 'btn_Diff') {
                    this.VM.openWindow("Differential", e.pid, e.pname);
                }
                else if (action === 'btn_Fclty') {
                    this.VM.openWindow("Facility", e.pid, e.pname);
                }
                else if (action === 'btn_Fclty_Img') {
                    this.VM.openWindow("Facility_Image", e.pid, e.pname);
                }
                else if (action === 'btn_Db_Loc') {
                    this.VM.openWindow("Distribusion_Location", e.pid, e.pname);
                }
                else if( action === 'btn_stn_Comb') {
                    this.VM.openWindow("Group_Station_Combinations", e.pid, e.pname);
                }
            }
        }
    }
    ...
    onCheckCopyLeadChange = (e, datamodel) => {
        if (this.VM) {
            const model = this.VM.Data;
            datamodel = datamodel || model;
            datamodel[e.target.name] = e.target.checked;
    ...
    setCopyLeadCheckBox = (e) => {
        const model = this.VM.Data;
        let value = null;
        if (e.row)
            value = e.row.cpy_lead_gst;
        return (
            <span>
                <input className="form-check-input" type="checkbox" name="cpy_lead_gst" checked={value} onChange={(ex) => this.onCheckCopyLeadChange(ex, e.row)} style={{ width: "20px", height: "20px" }} />
            </span>
        );
    };

    columnbtn = (e) => {
        const pid = e.row.crrg_id;
        const pname = e.row.crrg_nam;
        return (<div>
            <button type="button" style={{ width: "27px", height: "21px" }} name="btn_Edit" title={"Edit"} data-bs-toggle="tooltip" data-bs-placement="left" hot-key="H" className="btn btn-sm btn-icon1 btn-primary me-1" onClick={(e) => this.clickAction({ pname, pid, id: 'btn_Edit' })}><i className="fas fa-edit" style={{ fontSize: "10px", verticalAlign: "super" }}></i> </button>
            ...
            <button type="button" style={{ width: "27px", height: "21px" }} name="btn_Diff" hot-key="H" title={"Differential"} data-bs-toggle="tooltip" data-bs-placement="left" className="btn btn-sm btn-icon1 btn-primary me-1" onClick={(e) => this.clickAction({ pname, pid, id: 'btn_Diff' })}><i className="fas fa-users" style={{ fontSize: "10px", verticalAlign: "super" }}></i> </button>
            <button type="button" style={{ width: "27px", height: "21px" }} name="btn_Fclty" hot-key="H" title={"Facility"} data-bs-toggle="tooltip" data-bs-placement="left" className="btn btn-sm btn-icon1 btn-primary me-1" onClick={(e) => this.clickAction({ pname, pid, id: 'btn_Fclty' })}><i className="fas fa-concierge-bell" style={{ fontSize: "10px", verticalAlign: "super" }}></i> </button>
        </div>)
    };
    ...
    renderGrid() {
        ...
        gridInfo.Columns.forEach((e, i) => {
            if (i === 5)
                e.onRender = (_e) => this.setCopyLeadCheckBox(_e);
            if (i === 6)
                            ...
                            <div className="col-md-12">
                                <div className="row">
                                    <div className="col-md-4 mb-2">
                                        ...
                                        <cntrl.WKLTextbox mandatory={true} ref={(el) => this.onRefChange(el, 'CarriageName')} name="CarriageName" value={dataModel.CarriageName} onChange={this.onSearchChange} inputType={cntrl.WKLTextboxTypes.sentenceCase} maxLength={100}>
                                        </cntrl.WKLTextbox>
                                    </div>
                                    ...
                                    <div className="col-md-2 mb-2">
                                        <label className="form-label" >Carriage Delisted</label>
                                        <div className="col-md-2 mt-2">
                                            <input className="form-check-input" type="checkbox" name="Crrg_Aval_Srch" checked={dataModel.Crrg_Aval_Srch} onChange={this.onCheckChangeSrch} style={{ width: "20px", height: "20px" }} />
                                        </div>
                ...
                <div className="window-button-area">
                    <div className="row">
                        <div className="col border-start">
                            <button type="button" id="btn_Fclty_Img" hot-key="C" className="btn btn-sm btn-close1 btn-primary me-1" onClick={(e) => this.clickAction({ id: 'btn_Fclty_Img' })}><i className="fa fa-image"></i> Facilities images</button>
                            <button type="button" id="btn_Db_Loc" hot-key="C" className="btn btn-sm btn-close1 btn-primary me-1" onClick={(e) => this.clickAction({ id: 'btn_Db_Loc' })}><i className="fa fa-location"></i> Distribusion Location</button>
                            <button type="button" id="btn_stn_Comb" hot-key="C" className="btn btn-sm btn-close1 btn-primary me-1" onClick={(e) => this.clickAction({ id: 'btn_stn_Comb' })}><i className="fa fa-edit"></i> Group Station Combinations</button>
                        </div>
