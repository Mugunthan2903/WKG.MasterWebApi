...
export default class SSM034VM extends cntrl.VMBase {
    constructor(props) {
        super(props);
    ...
    init() {
        ...
        model.FormID = "SSM034";
        model.Title = '';
        model.prod_id = this.props.data.Prod_ID;
        model.prod_name = this.props.data.Prod_Name;
    ...
    onLoad() {
        ...
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM034LoadInitData034Async`, data: dataInfo, files: [] }, (r) => {
            model.Loading = false;
            if (r) {
                try {
    ...
    loadPage(pageIndex, columnOptions = null) {
        ...
        model.AllSelected = false;
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM034SearchDataAsync`, data: dataInfo }, (r) => {
            try {
                model.Loading = false;
    ...
    loadSelectedData(excptn_srl) {
        ...
        model.Loading = true;
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM034LoadSelectedDataAsync`, data: { excptn_srl } }, (r) => {
            model.Loading = false;
            if (r) {
    ...
    doSave(e) {
        if (this.isvalidSave(e)) {
            ...
            model.Loading = true;
            me.updateUI();
            cntrl.Utils.ajax({ url: `${this._WebApi}/SSM034SaveDataAsync`, data: dataInfo }, r => {
                try {
                    model.Loading = false;
                    r = r || {};
                    if (r.IsSuccess === true) {
    ...
    onBlurSearch() {
        ...
        dataInfo.supp_map_id = model.SuppMapID;
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM034BlurCheck`, data: dataInfo }, (r) => {
            try {
    ...
    setTitle() {
        const model = this.Data;
        if (model.Input.IsEdit) {
            model.Title = `Ltd Product / Exception / ${model.prod_name} / Edit ${cntrl.Utils.isNullOrEmpty(model.Input.group_name) ? "" : "/ " + model.Input.group_name.Text}`;
        } else {
            model.Title = `Ltd Product / Exception / ${model.prod_name} / New`;
        }
    };
