...
export default class SSM036VM extends VMBase {
    constructor(props) {
        super(props);
    ...
    newMode(flag = false) {
        ...
        dataModel.Prod_Name = '';
        //dataModel.Prod_Name = model.tui_prod_nam;
        //dataModel.Prod_ID = model.tui_prod_idF;
    ...
    loadInitData() {
        ...
        dataInfo.lang_cd = this.props.data.lang_cd;
        model.Loading = true;
        model.AllSelected = false;
        this.updateUI();
        cntrl.Utils.ajax({ url: `${this._WebApi}/SSM036LoadInitDataAsync`, data: dataInfo, files: [] }, (r) => {
            try {
                model.Loading = false;
                model.LanguageList = r.comboList.map((data) => ({ ID: data.lang_cd, Text: data.lang_nam }));
                if (model.Input.IsEdit) {
                    model.Input.Lang_SNO = r.data.lang_srl;
                    model.Input.Language = model.LanguageList.find(e => e.ID === r.data.lang_cd)
                    model.Input.Prod_Name = r.data.prod_nam;
                    model.Input.ModifiedOn = r.data.mod_dttm;
                    model.Input.ModifiedBy = r.data.mod_by_usr_cd;
                } else {
                }
            }
            catch (ex) {
    ...
    doSave(e) {
        if (this.isvalidSave(e)) {
            ...
            model.Loading = true;
            me.updateUI();
            Utils.ajax({ url: `${this._WebApi}/SSM036SaveDataAsync`, data: dataInfo }, r => {
                try {
                    model.Loading = false;
                    r = r || {};
                    if (r.IsSuccess === true) {
    ...
    handleModified(r) {
        ...
        this.showConfirmation("Record already exists.Do you want to retrieve?", false, (e) => {
            if (e == 0) {
                ...
                model.Input.Lang_SNO = r.lang_srl;
                model.Input.Language = model.LanguageList.find(e => e.ID === r.lang_cd)
                model.Input.Prod_Name = r.prod_nam;
    ...
    checkProdExist() {
        ...
        me.updateUI();
        Utils.ajax({ url: `${this._WebApi}/SSM036ProdLangExistAsync`, data: dataInfo }, r => {
            try {
                r = r || {};
                if (r.prod_exist) {
                    me.handleModified(r);
